...
class ASpellWorkerPool {
	...
	create(language) {
		if (this.PROCESS_POOL.length >= this.MAX_WORKERS) {
			logger.log({maxworkers: this.MAX_WORKERS}, "maximum number of workers already running");
			return null;
		}
	...
	check(language, words, timeout, callback) {
		...
		let worker;
		const availableWorker = _.find(this.PROCESS_POOL, cached => (cached.language === language) && cached.isReady());
		...
		worker.killTimer = setTimeout(() => worker.kill("spell check timed out")
		...
		return worker.check(words, (err, output) => {
			...
			callback(err, output);
			if (err != null) { return; } // process has shut down
