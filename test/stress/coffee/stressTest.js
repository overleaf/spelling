...
const wordlist = fs.readFileSync(WORDS).toString().split('\n').filter(w => w.match(/^[a-z]+$/));
...
const make_request = function(correctWords, incorrectWords, callback) {
	...
	return request.post('http://localhost:3005/user/1/check', {json:true, body: {words: full}}, function(err, req, body) {
		let m;
		...
		if (expected.length !== misspellings.length) {
			...
			for (i = 0, end = Math.max(expected.length, misspellings.length)-1, asc = 0 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
...
const q = async.queue((task, callback) =>
	setTimeout(() => make_request(task.correct, task.incorrect, callback)
	, Math.random() * 100)
